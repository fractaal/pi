...
export type SessionCompactionMode = "openai-native";
...
export interface OpenAINativeCompactionEntry extends SessionEntryBase {
...
): CompactionEntry | OpenAINativeCompactionEntry | null {
	for (let i = entries.length - 1; i >= 0; i--) {
		const entry = entries[i];
		if (entry.type === "compaction" || entry.type === "openai_native_compaction") return entry;
	}
	return null;
}
...
export function buildContextEntries(
	...
	for (let i = 0; i < compactionIdx; i++) {
		const entry = path[i];
		if (entry.id === checkpoint.firstKeptEntryId) {
			foundFirstKept = true;
		}
		if (foundFirstKept) {
...
export class SessionManager {
	...
	/** Append an opaque OpenAI native checkpoint as child of current leaf, then advance leaf. */
	appendOpenAINativeCompaction(
		modelId: string,
		item: OpenAINativeCompactionItem,
		tokensBefore: number,
		usage: Usage,
	...
	createBranchedSession(leafId: string): string | undefined {
		...
		const header: SessionHeader = {
			...
			compactionMode: this.getHeader()?.compactionMode,
		};
	...
	static forkFrom(
		...
		const newHeader: SessionHeader = {
			...
			compactionMode: sourceHeader.compactionMode,
		};
